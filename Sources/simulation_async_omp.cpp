...
void simulation(bool affiche)
{
    ...
    //contexte.taux_population = 400'000*4;
    ...
    if (rank==1)
    {
        ...
        while (1)
        {
            ...
            //# pragma omp parallel for reduction(+:compteur_grippe)
            ...
            //# pragma omp parallel for schedule(dynamic) reduction(+:compteur_agent)
            for ( auto& personne : population )
            {
            ...
            # pragma omp parallel for schedule(dynamic) reduction(+:mouru)
            for ( auto& personne : population )
            {
